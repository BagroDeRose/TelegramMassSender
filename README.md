...
**682 automated tests, 0 failures** (pytest + pytest-asyncio), run against a hand-built mock Telegram client (`tests/mocks/mock_telegram_client.py`) — no real Telegram account or network access needed. Coverage includes:
...
- Every change is covered by the automated test suite (682 tests, pytest/pytest-asyncio, mocked Telegram client).
...
Публикация релиза автоматизирована: пуш тега вида `vX.Y.Z` запускает `.github/workflows/release.yml`, который прогоняет полный набор тестов, собирает `.exe` через PyInstaller, упаковывает `dist/TelegramMassSender` в `TelegramMassSender-Windows.zip`, проверяет, что в сборку не попали секреты/файлы сессий/логи, считает SHA-256 и публикует GitHub Release с этим архивом и файлом контрольной суммы. Ручной процесс (`build_windows.bat` + `gh release create` вручную) остаётся рабочим как запасной вариант, если автоматический workflow по какой-то причине нужно обойти.
